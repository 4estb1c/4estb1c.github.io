(() => {'use strict';
  ...
  function renderFigure(figure){const chart=window.InspectorCharts?.render(figure);if(!chart)return null;const wrap=document.createElement('figure');wrap.className='chart';if(figure.title&&String(figure.title)!=='undefined'){const title=text(document.createElement('h4'),figure.title);title.className='chart-title';wrap.append(title)}if(figure.description&&String(figure.description)!=='undefined'){const description=text(document.createElement('p'),figure.description);description.className='chart-description';wrap.append(description)}if(chart.tagName==='svg'){const label=[figure.title,figure.description].filter(value=>value&&String(value)!=='undefined').join('. ');chart.setAttribute('aria-label',label||'Data chart');chart.setAttribute('tabindex','0')}wrap.append(chart);if(figure.source&&String(figure.source)!=='undefined')wrap.append(text(document.createElement('figcaption'),figure.source));return wrap}
  ...
  async function renderReader(){
    ...
    try{
      ...
      const contextUrl=article.context?.downloadUrl||'context/situational-awareness.md';
      $('#context-link').href=contextUrl;
      ...
      $('#copy-prompt').addEventListener('click',async()=>{
        try{
          const [promptResponse,contextResponse]=await Promise.all([fetch(article.context?.promptUrl||'prompts/discuss.md'),fetch(contextUrl)]);
...
${research}`);
      ...
      toggle.addEventListener('click',()=>{
        ...
        if(show)heading.scrollIntoView({behavior:'smooth',block:'start'});
      });
